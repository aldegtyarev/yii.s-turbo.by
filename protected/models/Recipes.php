...
class Recipes extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('page_id, product_id, product_name, dosage', 'required'),
			array('page_id, product_id', 'numerical', 'integerOnly'=>true),
			array('product_name', 'length', 'max'=>255),
			array('dosage', 'length', 'max'=>32),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, page_id, product_id, product_name, dosage', 'safe', 'on'=>'search'),
		);
	}
