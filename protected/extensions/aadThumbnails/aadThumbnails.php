...
class aadThumbnails
{
	...
	* Конструктор
	*/
	function aadThumbnails()
	{
		$this->path = Yii::app()->basePath."/extensions/aadThumbnails";
		require_once($this->path.DS.'aadthumbnails'.DS.'imgtag.class.php');
		//print_r($this->path.DS.'addthumbnails'.DS.'imgtag.class.php');
		/*
		...
		*/
		...
		if ((version_compare(PHP_VERSION, '5.0.0', '<'))) {
			echo ('Plugin mavikThumbnails needs PHP 5. You use PHP').' '.PHP_VERSION;
		}
		...
		*/
		...
		if($this->popupType) {
			$file = $this->path.DS.'aadthumbnails'.DS. $this->popupType.'.php';
			require_once( $file );
		}
		$type = 'aadThumbnailsDecorator' . $this->popupType;
		$this->decorator = new $type($this);
		$this->img = new aadThumbnailsImgTag();
		...
		$proportinsClass = 'aadThumbnailsProportions'.$proportions;
		require_once($this->path.DS.'aadthumbnails'.DS.'proportions'.DS.$proportions.'.php');
		
		
		$this->proportionsStrategy = new $proportinsClass($this);
		//echo'12121';
		//var_dump($this->decorator);
	}
	...
	*/
	...
	function setThumbName(&$thumbPath, &$thumbName)	
	{
		...
		if ($this->thumbPathSubdir == 1) {
		    ...
		    if(!$this->folderExists($path)) {
				//echo"<pre>";print_r($path);echo"</pre>";
				$indexFile = '<html><body bgcolor="#FFFFFF"></body></html>';
				$this->createFile($path, $indexFile, 'index.html');
	...
	function createFile($path, $fileContent, $fileName)
	{
		mkdir($path, 0777, true);
		$fp = fopen($path . DS . $fileName, "a"); // Открываем файл в режиме записи
...
class aadThumbnailsDecorator
{
	 ...
	 * @param $plugin
	 */
	function aadThumbnailsDecorator(&$plugin)
	{
		$this->plugin = $plugin;
	}
 ...
 * Родительский клас
 */
class aadThumbnailsProportions
{
	/**
	 * Ссылка на объект плагина
	 * @var plgContentMavikThumbnails
