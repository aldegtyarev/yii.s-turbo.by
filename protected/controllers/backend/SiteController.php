...
//		var_dump(Yii::app()->user->returnUrl);die;
	...
	/**
	 * страница после успешного обновления карты сайта.
	 */
	public function actionBuildsitemapfinished()
	{
		$urls_count = Yii::app()->request->getParam('rows', '0');
		
		$sitemap_file = Yii::getPathOfAlias('webroot') . '/sitemap.xml';
		$size = filesize($sitemap_file);
		$size = $this->FileSizeConvert($size);
		$this->render('buildsitemapfinished', array('urls_count' => $urls_count,'size' => $size));
	}
