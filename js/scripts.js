...
$(document).ready(function () {
	...
	var menu_cat_li_a = $("#categories-menu li ul li a"),
        ...
        timerId = null,
        $search_auto_form = null,
        $searchautoform = $('#searchautoform'),
		$centralCnt = $("#central-cnt"),
        $questionBnt = null,
        $questionFrmCnt = null;
    ...
    $search_auto_form = $('#search-auto-form');
	...
	$central_cnt.on('click', '#categories-menu li ul li a', function (e) {		//кликнули на пункт бокового меню
		...
		if (elem.length) {
			...
			if($(this).parent().hasClass('bodyset')) {
				...
				$search_auto_form.find('#select-year').val($(this).parent().data('body_id')).trigger('refresh');
				$search_auto_form.find('#select-year').change();
				return false;
			}
	...
	$central_cnt.on('click', "#categories-menu > li > a", function () {		//кликнули на главный пункт бокового меню
		return false;
	});
	
	$centralCnt.on('submit', '.product-comment-form', function () {
	    var form = $(this);
        // $questionFrmCnt = form.closest('.question-response-cnt');
        $questionFrmCnt = form.closest('div');
        $.post(
            form.attr('action'),
            form.serialize(),
            function (data) {
                $questionFrmCnt.html(data);
                var $showPopup = $questionFrmCnt.find('.fb-question-sended');
                if($showPopup.length > 0) {
                    $(".fb-question-sended").fancybox({padding : 0});
                    $showPopup.click();
                    // $questionFrmCnt.closest('.quection-form-cnt').hide();
                }
            }
        );
        return false;
	...
	$centralCnt.on('click', '.question-bnt', function () {
        $questionBnt = $(this);

        if(!$questionBnt.hasClass('active')) {
            $.ajax({
                type: 'get',
                url: '/product/comments/' + $(this).data('id'),
                // data: {model_id : $(this).val(), level : 1},
                beforeSend: function () {},
                success: function (msg) {
                    $questionBnt.parent().find('.question-response-cnt').html(msg);
                    $questionBnt.parent().find('.question-response-cnt').slideDown(300);
                }
            });

        } else {
            $questionBnt.parent().find('.question-response-cnt').slideUp(300, function () {
                $questionBnt.parent().find('.question-response-cnt').html('');
            });
        }

        $questionBnt.toggleClass('active');

    });

	$centralCnt.on('click', '.product-types-block a, .related_categories a', function (e) {
		e.preventDefault();
		$searchautoform.attr('action', $(this).attr('href'));		
		History.pushState(null, document.title, $(this).attr('href'));
		scroll_el = $centralCnt;
		scroll_el_pos = scroll_el.offset().top;
		scroll_to_block = false;
	});
	
	$centralCnt.on('click', '.related_categories a', function (e) {
		var url = $(this).attr('href'),
			link = $central_cnt.find('#categories-menu li ul li.'+$(this).data('cat')).children('a');
		e.preventDefault();
		link.attr('href', url);
		link.click();
		return false;
	...
	$centralCnt.on('click', '.page-item-more, .page-readmode, .all-items, .page-title, .page-image, .product-title, .product-detail, .switch-view', function (e) {
		e.preventDefault();
		History.pushState(null, document.title, $(this).attr('href'));
		scroll_el = $('#search-auto-block');
		scroll_el_pos = scroll_el.offset().top;
	...
	$centralCnt.on('click', '.addtocart', function (e) {		//добавляем товар в корзину
		e.preventDefault();
    ...
    $search_auto_form.on('change', '#select-marka', function () {
		...
		$.ajax({
			...
			data: {model_id : $(this).val(), level : 1},
			beforeSend: function () {
                timerId = setTimeout(function() { $('#select-model-loading').show(); }, 1000);
            },
			success: function (msg) {
				...
				$('#select-year-wr').find('select').styler();
			}
		});
    });
	
    $search_auto_form.on('change', '#select-model', function () {
		$(this).parent().parent().parent().addClass('step-selected');
		$.ajax({
			type: 'post',
			url: '/shopmodelsauto/descendants',
			...
			beforeSend: function () {
                timerId = setTimeout(function() { $('#select-year-loading').show(); }, 1000);
            },
			success: function (msg) {
                clearTimeout(timerId);
    ...
    $search_auto_form.on('change', '#select-year', function () {
		if($(this).val() != '') {
			$(this).parent().parent().parent().addClass('step-selected');
    ...
    $search_auto_form.on('click', '#clear-search-auto', function () {
	...
	$centralCnt.on('mouseover', '.product-list-item-row .product-image', function () {
	...
	$centralCnt.on('mouseover', ".add-prod-img", function () {
	...
	$centralCnt.on('mouseout', '.product-list-item-row .product-image, .add-prod-img', function () {
		hovered = false;
		//clearTimeout(timeOut);
		//clearTimeout(checkhover);
	...
	$centralCnt.on('mouseover', '.product-detail-tile', function () {
		$(this).parent().parent().parent().find('.product-title').css('text-decoration','underline');
	});
	$centralCnt.on('mouseout', '.product-detail-tile', function () {
	...
	$central_cnt.on('click', '#showMore', function() {
		...
		if (!loadingFlag)
		{
			...
			$central_cnt.find('#loading').show();
			$central_cnt.find('#showMore').hide();
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					$central_cnt.find('#loading').hide();
					$central_cnt.find('#showMore').show();

					// вставляем полученные записи после имеющихся в наш блок
					$central_cnt.find('#listView').append(data);
	...
	function loadPage(url) {
		...
		if(model_is_change === true) {
			if(is_bodyset_change === true) {
				block_to_load = $centralCnt.find("#categories-menu");
		...
		block_to_load.load(url + cnt_to_load, function(responseTxt) {
			...
			if ($(scroll_el).length != 0) {
				if(scroll_to_block === true) {
					if($('#content-wr').find('#scroll_to_engine').val() == '1') {
						...
						$('#select_engine_notice').find('span').text(select_str);
			...
			if ($central_cnt.find('#pageCount_input').length != 0) {
				page = parseInt($central_cnt.find('#page_input').val());
				pageCount = parseInt($central_cnt.find('#pageCount_input').val());
				csrfTokenName = $central_cnt.find('#csrfTokenName').val();
				csrfToken = $central_cnt.find('#csrfToken').val();
				pagination_url = $central_cnt.find('#pagination-url').val();
			}
			
			if ($central_cnt.find('.pager').length != 0) {
				$central_cnt.find('.pager').hide();
			}
